  ...
  return (
    <div className="space-y-6">
      ...
      <StatGrid cols={4}>
        ...
        />
        <StatCard
          title="Receitas Críticas"
          value={criticalRecipes}
          icon={AlertTriangle}
        ...
        <ContentCard padding="lg" className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <h3 className="mt-1 text-xl font-black tracking-tight text-slate-900">
                Receitas prontas para reproduzir
              </h3>
            </div>
          ...
          {loading ? (
            <div className="rounded-3xl border border-dashed border-slate-200 bg-slate-50/70 px-6 py-16 text-center text-xs font-black uppercase tracking-[0.24em] text-slate-400">
              Carregando receitas e histórico...
            </div>
            ...
            <GridTable
              ...
              keyExtractor={(row) => row.recipe.id}
              noDesktopCard
              renderMobileItem={({ recipe, simulation }) => (
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <p className="truncate text-sm font-black text-slate-900">{recipe.name}</p>
                    {!recipe.active && <Badge color="default">Inativa</Badge>}
                    {simulation.missingItems > 0 && <Badge color="warning">Falta estoque</Badge>}
                  </div>
              ...
              renderMobileExpandedContent={({ recipe, simulation }) => (
                <div className="space-y-4 px-4 py-4">
                  <div className="grid grid-cols-2 gap-3">
                    <div className="rounded-2xl bg-slate-50 p-3">
                      <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Custo base</p>
                      <p className="mt-1 text-sm font-black text-slate-900">{formatCurrency(simulation.totalCost)}</p>
                    </div>
                    <div className="rounded-2xl bg-slate-50 p-3">
                      <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">CMV unitário</p>
                      <p className="mt-1 text-sm font-black text-slate-900">{formatCurrency(simulation.costPerOutput)}</p>
                    </div>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <Button size="sm" variant="secondary" onClick={() => setRecipeToProduce(recipe)}>
                      Produzir
                    </Button>
                    <Button
                      size="sm"
                    ...
                    <Button
                      size="sm"
                      variant="danger"
                      onClick={() => void handleDeleteRecipe(tenant.slug, recipe, fetchData)}
                    >
                      Excluir
                    </Button>
                  </div>
              ...
              getMobileBorderClass={({ recipe, simulation }) => {
                if (!recipe.active) return "border-slate-200";
                return simulation.missingItems > 0 ? "border-amber-200" : "border-emerald-200";
              }}
              columns={[
                {
                  header: "Receita",
                  render: ({ recipe, simulation }) => (
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <p className="text-sm font-black text-slate-900">{recipe.name}</p>
                        {!recipe.active && <Badge color="default">Inativa</Badge>}
                        {simulation.missingItems > 0 && <Badge color="warning">Falta estoque</Badge>}
                      </div>
                      <p className="text-[11px] font-semibold text-slate-500">
                ...
                },
                {
                  ...
                  render: ({ simulation }) => (
                    <div className="space-y-1">
                      <p className="text-xs font-black text-slate-900">{formatCurrency(simulation.totalCost)}</p>
                      <p className="text-[11px] text-slate-500">
                        {formatCurrency(simulation.costPerOutput)} por {simulation.outputUnit}
                      </p>
                    </div>
                  ),
                },
                {
                  ...
                  render: ({ recipe, simulation }) => (
                    <span className="text-xs font-semibold text-slate-600">
                      {recipe.ingredients.length} insumo(s) • {simulation.overheads.length} custo(s)
                    </span>
                  ),
                },
                {
                  ...
                  render: ({ recipe }) => (
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <IconButton
                        ...
                        onClick={(event) => {
                          event.stopPropagation();
                          void handleDeleteRecipe(tenant.slug, recipe, fetchData);
                        }}
                      >
        ...
        <ContentCard padding="lg" className="space-y-5">
          {!selectedRecipe || !previewSimulation ? (
            <EmptyState
              ...
              description="Ao selecionar uma ficha técnica você verá consumo projetado, custo total, CMV unitário e entrada automática do produto final."
            />
          ) : (
            <>
                ...
                <div className="space-y-2">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {previewSimulation.missingItems > 0 && (
                      <Badge color="warning">
                        {previewSimulation.missingItems} item(ns) em falta
                      </Badge>
                    )}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {[1, 2, 5].map((multiplier) => (
                      <Button
                        ...
                        variant="outline"
                        onClick={() =>
                          setPreviewQuantity(String(roundProductionValue(selectedRecipe.outputQuantity * multiplier)))
                        }
                      >
                        x{multiplier}
                      </Button>
              ...
              <div className="grid grid-cols-2 gap-3">
                <SummaryTile
                  ...
                  value={formatCurrency(previewSimulation.totalIngredientCost)}
                  icon={<Package className="h-4 w-4" />}
                />
                <SummaryTile
                  label="Custos indiretos"
                  value={formatCurrency(previewSimulation.totalOverheadCost)}
                  icon={<CircleDollarSign className="h-4 w-4" />}
                />
                <SummaryTile
                  label="Custo total"
                  value={formatCurrency(previewSimulation.totalCost)}
                  icon={<ChefHat className="h-4 w-4" />}
                />
                <SummaryTile
                  label="CMV por unidade"
                  value={formatCurrency(previewSimulation.costPerOutput)}
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-sm font-semibold text-emerald-800">
                        Preço do produto: {formatCurrency(selectedRecipe.product.price)}
                      </p>
                    </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500">
                      Conversão automática para a unidade do estoque e baixa precisa no registro.
                    </p>
                  </div>
                </div>
                      ...
                      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                        <div className="space-y-1">
                          ...
                          <p className="text-[11px] font-semibold text-slate-500">
                            ...
                            {ingredient.convertedQuantity !== null
                              ? ` • Estoque: ${formatQuantity(ingredient.convertedQuantity, ingredient.inventoryUnit)}`
                              : ""}
                          </p>
                          {ingredient.message && (
                            <p className="text-[11px] font-semibold text-amber-700">{ingredient.message}</p>
                          )}
                        </div>

                        <div className="grid grid-cols-2 gap-3 text-right sm:min-w-[240px]">
                          <MetricPill
                            label="Antes"
                            value={formatQuantity(ingredient.stockBefore, ingredient.inventoryUnit)}
                          />
                          <MetricPill
                            label="Depois"
                            value={formatQuantity(ingredient.stockAfter, ingredient.inventoryUnit)}
                          />
                          <MetricPill
              ...
              <div className="space-y-3">
                <p className="text-sm font-black text-slate-900">Custos indiretos da produção</p>
                {previewSimulation.overheads.length === 0 ? (
                  <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50/60 px-4 py-5 text-sm text-slate-500">
                    Nenhum custo adicional cadastrado. Você pode incluir energia, água, gás, mão de obra e embalagem.
                  </div>
                ) : (
                  <div className="space-y-3">
                    {previewSimulation.overheads.map((overhead) => {
                      ...
                      return (
                        <div key={overhead.id} className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 sm:flex-row sm:items-center sm:justify-between">
                          <div className="space-y-1">
                            <div className="flex flex-wrap items-center gap-2">
                              <Badge color={accent.color} icon={accent.icon}>
                                {overhead.label}
                              </Badge>
                              <span className="text-[11px] font-semibold text-slate-500">
                                {overhead.calculationMode === "PER_OUTPUT_UNIT"
                                  ? "Aplicado por unidade produzida"
                                  : "Aplicado por receita base"}
                              </span>
                            </div>
                            {overhead.notes && (
                              <p className="text-[11px] text-slate-500">{overhead.notes}</p>
                            )}
                          </div>
                          <div className="text-right">
                            <p className="text-sm font-black text-slate-900">{formatCurrency(overhead.totalCost)}</p>
                            <p className="text-[11px] text-slate-500">
                              Base {formatCurrency(overhead.cost)}
                            </p>
                          </div>
                        </div>
                      );
                  ...
                  <div className="rounded-3xl border border-[#0D1B3E]/10 bg-[#0D1B3E]/[0.03] p-4">
                    <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                      <div>
                        ...
                        <p className="text-[11px] text-slate-500">
                          Ao concluir a produção, o item final vinculado ao cardápio também recebe entrada automática.
                        </p>
                      </div>
                    ...
                    <div className="mt-4 grid gap-3 md:grid-cols-2">
                      <MetricPill
                        ...
                        value={formatQuantity(
                          previewSimulation.outputSnapshot.requestedQuantity,
                          previewSimulation.outputSnapshot.requestedUnit,
                        )}
                      />
                      <MetricPill
                        label="Estoque final"
                        value={
                          previewSimulation.outputSnapshot.convertedQuantity !== null
                            ? formatQuantity(
                                previewSimulation.outputSnapshot.convertedQuantity,
                                previewSimulation.outputSnapshot.inventoryUnit,
                              )
                            : "Sem conversão"
                        }
                      />
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                <Button
                  ...
                  size="lg"
                  iconLeft={<Play className="h-4 w-4" />}
                  onClick={() => setRecipeToProduce(selectedRecipe)}
                >
                  Registrar Agora
                </Button>
                <Button
      ...
      <ContentCard padding="lg" className="space-y-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h3 className="mt-1 text-xl font-black tracking-tight text-slate-900">Últimas produções registradas</h3>
          </div>
          <p className="text-xs text-slate-500">
            Cada registro mantém os custos, o consumo e a movimentação do momento em que foi produzido.
          </p>
        </div>
          ...
          <GridTable
            ...
            renderMobileExpandedContent={(run) => (
              <div className="space-y-4 px-4 py-4">
                ...
                {run.createdByName && (
                  <p className="text-[11px] font-semibold text-slate-500">
                    Produzido por {run.createdByName}
                  </p>
                )}
              ...
              },
              {
                ...
                render: (run) => (
                  <span className="text-xs font-black text-slate-900">
                    {formatQuantity(run.quantityProduced, run.unit)}
                  </span>
                ),
              },
              {
                ...
                render: (run) => (
                  <div className="space-y-1">
                    ...
                    <p className="text-[11px] text-slate-500">
                      {formatCurrency(run.costPerOutput)} por {run.unit}
                    </p>
                  </div>
                ),
              },
...
async function handleDeleteRecipe(slug: string, recipe: ProductionRecipe, refresh: () => Promise<void>) {
  ...
  try {
    await apiJson(`/api/tenants/${slug}/production/recipes/${recipe.id}`, {
      method: "DELETE",
    });
    await refresh();
  } catch (error) {
...
function RecipeEditorModal({
  ...
  const [overheads, setOverheads] = useState<ProductionRecipeOverhead[]>(
    recipe?.overheads || [],
  );
  ...
  const addOverhead = () => {
    setOverheads((current) => [
      ...current,
      {
        id: createRowId("overhead"),
        label: "",
        type: "ENERGIA",
        cost: 0,
        calculationMode: "PER_RECIPE",
        notes: "",
      },
    ]);
  };
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      const payload = {
        ...
        overheads: overheads.map((overhead) => ({
          ...overhead,
    ...
    >
      <form id="production-recipe-form" onSubmit={handleSubmit} className="space-y-6">
        <div className="rounded-3xl border border-slate-200 bg-slate-50/70 p-4">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-[11px] text-slate-500">
                Dê nome, descreva o rendimento e vincule ao produto final do cardápio.
              </p>
            </div>
          ...
          <div className="mt-4 space-y-4">
            <Input
              ...
              placeholder="Ex.: Bolo de cenoura premium"
              required
            />
            <Textarea
              label="Descrição"
              ...
              placeholder="Explique o que esta produção gera, em que contexto é usada e qual o padrão desejado."
            />
            <FormRow cols={3}>
              <Select
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    {ingredients.length > 1 && (
                      <IconButton
                        ...
                        variant="ghost"
                        className="text-red-500 hover:text-red-700"
                        onClick={() => setIngredients((current) => current.filter((row) => row.id !== ingredient.id))}
                      >
                        <Trash2 className="h-4 w-4" />
                      </IconButton>
                    )}
                  ...
                  <div className="mt-4 space-y-4">
                    <FormRow cols={3}>
                      <Select
                        ...
                        onChange={(event) => {
                          const item = inventoryMap.get(event.target.value);
                          setIngredients((current) =>
                            current.map((row) =>
                ...
                <div key={overhead.id} className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <IconButton
                      ...
                      variant="ghost"
                      className="text-red-500 hover:text-red-700"
                      onClick={() => setOverheads((current) => current.filter((row) => row.id !== overhead.id))}
                    >
                      <Trash2 className="h-4 w-4" />
                    </IconButton>
                  </div>
                      ...
                      <Input
                        ...
                        onChange={(event) =>
                          setOverheads((current) =>
                            current.map((row) =>
                              row.id === overhead.id
                                ? { ...row, cost: Number(event.target.value) }
                                : row,
                            ),
                          )
                        }
                      ...
                      <Select
                        ...
                        onChange={(event) =>
                          setOverheads((current) =>
                            current.map((row) =>
                              row.id === overhead.id
                                ? { ...row, type: event.target.value as ProductionOverheadType }
                                : row,
                            ),
                          )
                        }
                      ...
                      <Select
                        ...
                        onChange={(event) =>
                          setOverheads((current) =>
                            current.map((row) =>
                              row.id === overhead.id
                                ? { ...row, calculationMode: event.target.value as ProductionRecipeOverhead["calculationMode"] }
                                : row,
                            ),
                          )
                        }
        ...
        <div className="rounded-3xl border border-slate-200 bg-slate-50/70 p-4">
          ...
          <p className="text-[11px] text-slate-500">
            Registre o processo para reproduzir a mesma produção com consistência.
          </p>
          <Textarea
      ...
      footer={(
        <ModalFooter>
          ...
          <Button
            ...
            loading={saving}
            disabled={simulation.missingItems > 0}
          >
            Confirmar Produção
          </Button>
        </ModalFooter>
    ...
    >
      <form id="production-run-form" onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          <SummaryTile
            label="Insumos"
            value={formatCurrency(simulation.totalIngredientCost)}
            icon={<Package className="h-4 w-4" />}
          />
          <SummaryTile
            label="Custos indiretos"
            value={formatCurrency(simulation.totalOverheadCost)}
            icon={<CircleDollarSign className="h-4 w-4" />}
          />
          <SummaryTile
            label="Custo total"
            value={formatCurrency(simulation.totalCost)}
            icon={<ChefHat className="h-4 w-4" />}
          />
          <SummaryTile
            label="CMV unitário"
            value={formatCurrency(simulation.costPerOutput)}
            icon={<Factory className="h-4 w-4" />}
          />
        </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-black text-amber-900">
                  Produção bloqueada até ajustar o estoque
                </p>
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="text-[11px] text-slate-500">
                    ...
                    {ingredient.convertedQuantity !== null
                      ? ` • baixa ${formatQuantity(ingredient.convertedQuantity, ingredient.inventoryUnit)}`
                      : ""}
                  </p>
                  {ingredient.message && (
                    <p className="mt-1 text-[11px] font-semibold text-amber-700">{ingredient.message}</p>
                  )}
                </div>
                <div className="text-right">
                  <p className="text-sm font-black text-slate-900">{formatCurrency(ingredient.totalCost)}</p>
                  <p className="text-[11px] text-slate-500">
                    saldo apÃ³s: {formatQuantity(ingredient.stockAfter, ingredient.inventoryUnit)}
                  </p>
                </div>
              </div>
            </div>
...
function ProductionRunDetailsModal({
  run,
  onClose,
}: {
  run: ProductionRun;
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Produzido em {formatDateTime(run.createdAt)}
                {run.createdByName ? ` por ${run.createdByName}` : ""}
              </p>
              {run.notes && (
              ...
              <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                <div className="space-y-1">
                  ...
                  <p className="text-[11px] text-slate-500">
                    Receita: {formatQuantity(ingredient.requestedQuantity, ingredient.unit)}
                    {ingredient.convertedQuantity !== null
                      ? ` • baixa real ${formatQuantity(ingredient.convertedQuantity, ingredient.inventoryUnit)}`
                      : ""}
                  </p>
                </div>
          ...
          <div className="rounded-3xl border border-[#0D1B3E]/10 bg-[#0D1B3E]/[0.03] p-4">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              <MetricPill
                ...
                value={formatQuantity(run.outputSnapshot.requestedQuantity, run.outputSnapshot.requestedUnit)}
              />
              <MetricPill
                label="Entrada no estoque"
                value={
                  run.outputSnapshot.convertedQuantity !== null
            ...
            {run.outputSnapshot.stockAfter !== undefined && run.outputSnapshot.stockAfter !== null && (
              <div className="mt-3">
                <MetricPill
                  label="Saldo final do item"
                  value={formatQuantity(run.outputSnapshot.stockAfter, run.outputSnapshot.inventoryUnit)}
                />
              </div>
            )}
